...
export default function NonVegBookingModal({ item, mealType, tomorrowDate, user, onClose, onSuccess }) {
    ...
    const checkBooking = async () => {
      try {
        const res = await axios.get(
          `${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/payment/status?studentId=${user.id}&date=${tomorrowDate}`
        );
        if (res.data.success) {
  ...
  const handlePay = async () => {
    ...
    try {
      const { data } = await axios.post(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/payment/create-order`, {
      ...
      const options = {
        ...
        handler: async (response) => {
          try {
            const verifyRes = await axios.post(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/payment/verify`, {
  ...
  const handleMockPay = async () => {
    setStep("paying");
    try {
      const res = await axios.post(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/payment/mock-success`, {
        studentId: user.id, studentName: user.name, studentEmail: user.email,
        date: tomorrowDate, mealType, item
      });
